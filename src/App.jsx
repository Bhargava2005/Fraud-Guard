...
import { fetchDashboardData } from './utils/dataLoader';
...
function App() {
  ...
  const [activeTab, setActiveTab] = useState('home'); // home, overview, eda, simulation
  ...
  return (
    <div className="min-h-screen flex bg-[var(--bg-light)] text-[var(--text-primary)] font-sans overflow-x-hidden">
      ...
      {/* Main Content Area */}
      <main className="flex-1 overflow-y-auto p-4 md:p-8 bg-[var(--bg-light)] pb-24 w-full">
        {activeTab !== 'home' && (
          ...
          <div className="space-y-6">
            ...
            <div className="panel p-8 bg-white max-w-7xl animate-fade-in">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-y-10 gap-x-8">
                <div>
                  <span className="block text-sm text-[var(--accent-blue)] font-bold uppercase tracking-wider mb-2">Total Rows</span>
                  <span className="text-2xl font-semibold text-gray-800">{data.systemInsights.totalDatasetSize}</span>
                </div>
                <div>
                  <span className="block text-sm text-[var(--accent-blue)] font-bold uppercase tracking-wider mb-2">Unique Users</span>
                  <span className="text-2xl font-semibold text-gray-800">{data.systemInsights.uniqueUsers}</span>
                </div>
                <div>
                  <span className="block text-sm text-[var(--accent-blue)] font-bold uppercase tracking-wider mb-2">Unique Products</span>
                  <span className="text-2xl font-semibold text-gray-800">{data.systemInsights.uniqueProducts}</span>
                </div>
                <div>
                  <span className="block text-sm text-[var(--accent-blue)] font-bold uppercase tracking-wider mb-2">Total Returns</span>
                  <span className="text-2xl font-semibold text-gray-800">{data.systemInsights.totalReturns}</span>
                </div>
                <div>
                  <span className="block text-sm text-[var(--accent-blue)] font-bold uppercase tracking-wider mb-2">Global Return Rate</span>
                  <span className="text-2xl font-semibold text-gray-800">{data.systemInsights.globalReturnRate}</span>
                </div>
                <div>
                  <span className="block text-sm text-[var(--accent-blue)] font-bold uppercase tracking-wider mb-2">Platforms Scanned</span>
                  <span className="text-lg font-medium text-gray-700">{data.systemInsights.platformsIncluded}</span>
                </div>
                <div>
                  <span className="block text-sm text-[var(--accent-blue)] font-bold uppercase tracking-wider mb-2">GNN Training Time</span>
                  <span className="text-2xl font-semibold text-gray-800">{data.systemInsights.trainingTime}</span>
                </div>
                <div>
                  <span className="block text-sm text-[var(--accent-blue)] font-bold uppercase tracking-wider mb-2">Inference Speed</span>
                  <span className="text-xl font-semibold text-gray-800">{data.systemInsights.inferenceTime}</span>
                </div>
              </div>
            </div>
        ...
        {/* EDA Tab */}
        {activeTab === 'eda' && (
          <div className="space-y-12 animate-fade-in w-full max-w-[1600px]">
            {/* Top Row: Extra Charts */}
            <div className="w-full">
              <AdditionalCharts platformData={data.platformDistribution} hotspotsData={data.hotspots} />
            </div>

            {/* Middle Row: Images and Model Deep Dives */}
            ...
            {/* Bottom Row: Trajectory & Project Report Diagrams */}
