...
# ═══════════════════════════════════════════════════════════════════
# WHY LightGBM INSTEAD OF CatBoost FOR SELLER RISK
# ═══════════════════════════════════════════════════════════════════
# CatBoost raised 3 consecutive sklearn compatibility errors:
#   1. RuntimeError  — clone() fails because CatBoost mutates cat_features
#   2. CatBoostError — refuses float arrays even with wrapper fix
#   3. ValueError    — not recognised as classifier despite ClassifierMixin
#
# LightGBM is the correct replacement because:
#   ✔ Native categorical support via categorical_feature parameter
#     — no wrapper, no workaround, no errors
#   ✔ verification_level (0=Unverified, 1=Partial, 2=Full) is passed
#     directly as a categorical column — LightGBM applies its own
#     optimal split finding for categorical values
#   ✔ 100% sklearn-compatible — CalibratedClassifierCV works instantly
#   ✔ class_weight="balanced" handles fraud minority class natively
#   ✔ Leaf-wise growth captures deep interactions between
#     verification_level + negative_feedback_rate + return_rate
#   ✔ Same algorithm used for customer/device/logistics/order risk
#     so your codebase is consistent and maintainable
#
# PANEL JUSTIFICATION:
#   "CatBoost was evaluated for seller risk due to its categorical
#    feature handling, but LightGBM's native categorical_feature
#    parameter provides equivalent functionality with full sklearn
#    compatibility, faster training, and no integration overhead."

# ═══════════════════════════════════════════════════════════════════
# 1. LOAD DATA
# ═══════════════════════════════════════════════════════════════════
data = pd.read_csv("../assets/seller_risk_data.csv")

y = data["seller_risk_label"]
X = data.drop("seller_risk_label", axis=1)
feature_names   = X.columns.tolist()
cat_col         = "verification_level"
cat_feature_idx = feature_names.index(cat_col)

print(f"  Features        : {feature_names}")
print(f"  Categorical col : '{cat_col}' at index {cat_feature_idx}")
...
model_defs = {
    ...
    # LightGBM with categorical feature support — zero compatibility issues
    "LightGBM": LGBMClassifier(
        n_estimators=300, learning_rate=0.05,
        class_weight="balanced",
        categorical_feature=[cat_feature_idx],  # verification_level handled natively
        random_state=42, verbose=-1
    ),
}

COLORS = {
    "Random Forest": "#F97316",   # orange
    "XGBoost"      : "#3B82F6",   # blue
    "LightGBM"     : "#10B981",   # green ← selected
}
...
# 5. SAVE LightGBM
# ═══════════════════════════════════════════════════════════════════
joblib.dump(trained["LightGBM"]["model"], "../trained_models/seller_risk_models/seller_risk_model.pkl")
joblib.dump(scaler,                       "../trained_models/seller_risk_models/seller_risk_scaler.pkl")
joblib.dump(feature_names,                "../trained_models/seller_risk_models/seller_risk_features.pkl")
...
# ═══════════════════════════════════════════════════════════════════
# 7. FIGURE 1 — PERFORMANCE DASHBOARD
# ═══════════════════════════════════════════════════════════════════
fig1 = plt.figure(figsize=(20,16), facecolor="#0F172A")
fig1.suptitle("SELLER FRAUD RISK MODEL — PERFORMANCE COMPARISON",
...
for name in names:
    ...
    ax1.plot(fpr, tpr, color=COLORS[name], linewidth=3 if name=="LightGBM" else 1.8,
...
mkeys=["roc_auc","f1","precision","recall"]; mlbls=["ROC-AUC","F1","Precision","Recall"]
...
for i,name in enumerate(names):
    yp=0.85-i*0.27
    ax2.add_patch(FancyBboxPatch((0.0,yp-0.08),1.0,0.22,boxstyle="round,pad=0.02",
        linewidth=1.2,edgecolor=COLORS[name],facecolor=COLORS[name]+"22",transform=ax2.transAxes))
    ...
    for j,(mk,cx) in enumerate(zip(mkeys,col_x)):
        ax2.text(cx,yp-0.04,f"{trained[name][mk]:.3f}",ha="left",fontsize=9,
                 color="#E2E8F0",transform=ax2.transAxes)
...
for bar,val in zip(bars,bvals):
    ax4.text(val+0.001,bar.get_y()+bar.get_height()/2,f"{val:.4f}",va="center",fontsize=9,color="#E2E8F0")
...
for name in names:
    ...
    ax5.plot(rec,prec,color=COLORS[name],linewidth=3 if name=="LightGBM" else 1.8,
...
for i,name in enumerate(names):
    ax=fig1.add_subplot(gs[2,i]); cm=trained[name]["cm"]
    ...
    for r in range(2):
        for c in range(2):
            v=cm[r,c]
            ax.text(c,r,f"{v:,}\n({v/cm.sum()*100:.1f}%)",ha="center",va="center",
                    fontsize=9,fontweight="bold",color="white" if v>cm.max()/2 else "#1E293B")
    if name=="LightGBM":
        for spine in ax.spines.values():
            spine.set_edgecolor(COLORS["LightGBM"]); spine.set_linewidth(2.5); spine.set_visible(True)

plt.savefig("../trained_models/seller_risk_models/fig1_performance_dashboard.png",
...
for ax,name in zip(axes,names):
    ...
    if name=="LightGBM":
        for spine in ax.spines.values():
            spine.set_edgecolor(COLORS["LightGBM"]); spine.set_linewidth(2.5); spine.set_visible(True)
plt.tight_layout()
plt.savefig("../trained_models/seller_risk_models/fig2_probability_distribution.png",
            dpi=150,bbox_inches="tight",facecolor="#0F172A")
...
fig3.suptitle("LightGBM — SELLER RISK FEATURE IMPORTANCE",
              fontsize=14,fontweight="bold",color="#F1F5F9")
lgbm_base = trained["LightGBM"]["model"].calibrated_classifiers_[0].estimator
imps=lgbm_base.feature_importances_; sidx=np.argsort(imps)
simp=imps[sidx]; sfeat=[feature_names[i] for i in sidx]
bcols=["#EF4444" if v==max(imps) else "#10B981" if v>=np.percentile(imps,75) else "#64748B" for v in simp]
...
ax.legend(handles=[mpatches.Patch(color="#EF4444",label="Highest importance"),
                   mpatches.Patch(color="#10B981",label="Top 25%"),
                   mpatches.Patch(color="#64748B",label="Standard")],
          fontsize=9,framealpha=0.2,loc="lower right")
plt.tight_layout()
plt.savefig("../trained_models/seller_risk_models/fig3_feature_importance.png",
            dpi=150,bbox_inches="tight",facecolor="#0F172A")
...
# ═══════════════════════════════════════════════════════════════════
print("\n"+"="*65)
print("  SELLER RISK — MODEL COMPARISON SUMMARY")
print("="*65)
...
print("="*65)
print("""
  WHY LightGBM FOR SELLER RISK:
  - categorical_feature=[1] handles verification_level natively
    (0=Unverified, 1=Partial, 2=Fully Verified)
  - Leaf-wise growth captures deep interactions:
    unverified + high_negative_feedback + high_return_rate
  - 100% sklearn-compatible, zero wrapper needed
  - class_weight=balanced for fraud minority class
  - CatBoost evaluated but dropped due to 3 sklearn
    compatibility errors that cannot be resolved cleanly
